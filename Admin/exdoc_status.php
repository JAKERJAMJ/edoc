        ...
        if (isset($_POST['SubmitDe'])) {
            ...
            if (isset($_POST['departments'])) {
                ...
                foreach ($selectedDepartments as $departmentId) {
                    // Fetch all users in the selected department
                    $userIds = [];
                    $sqlUserIds = "SELECT user_id FROM user WHERE de_id = '$departmentId'";
                    $resultUserIds = mysqli_query($con, $sqlUserIds);
                    while ($rowUserId = mysqli_fetch_array($resultUserIds)) {
                        $userIds[] = $rowUserId['user_id'];
                    }
